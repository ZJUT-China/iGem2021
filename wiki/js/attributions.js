...
var container = `<div class="text-big text-green title-big target-title-1">
Team Members
</div>`;
for (var group of members) {
    console.log(group);

    for (var item of group) {
        container += `<div><h4>${item.name}</h4><p>${item.attribution}</p></div>`;
    }
}
$(".members").html(container);
$("p").addClass("text-little");
$("h4").addClass("text-weight-2");